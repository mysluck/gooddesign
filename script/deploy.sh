...
#项目路径统一前缀(全路径)
prefix=/usr/local/design
#日志目录
logdir=/logs

#项目根路径
projectrootdir=${prefix}/
#生成的jar包的名称以及路径，保证sh文件到处都可运行，使用全路径
newjarfiledir=${prefix}/install
newjarfilename=${newjarfiledir}/jeecg-system-start-3.5.3.jar
#旧jar包不删除，移动到指定目录(为了不重名，采用时间串)
oldjarfiledir=${prefix}/install/oldjar
currenttime=$(date "+%Y%m%d%H%M%S")
oldjarfilename=${oldjarfiledir}/goodDesign${currenttime}.jar
...
if [ ! -d "${projectrootdir}" ]; then
	...
	echo "拉gitee上的项目 git clone https://github.com/mysluck/gooddesign.git"
